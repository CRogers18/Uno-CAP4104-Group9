...
public class CardOps {
	...
	public static Stack<Card> shuffle(Card[] deck, int deckLength ,int playerCount)
	{
		...
		if (deckLength == 108)
		{
			while (playerCount == 2 && deck[93].color == 'x')
			{
				int j = ThreadLocalRandom.current().nextInt(93, deckLength);
				Card temp = deck[j];
				deck[j] = deck[93];
				deck[14] = temp;
			}
			
			while (playerCount == 3 && deck[86].color == 'x')
			{
				int j = ThreadLocalRandom.current().nextInt(86, deckLength);
				Card temp = deck[j];
				deck[j] = deck[86];
				deck[21] = temp;
			}

			while (playerCount == 4 && deck[79].color == 'x')
			{
				int j = ThreadLocalRandom.current().nextInt(79, deckLength);
				Card temp = deck[j];
				deck[j] = deck[79];
				deck[28] = temp;
			}
			
			while (playerCount == 5 && deck[72].color == 'x')
			{
				int j = ThreadLocalRandom.current().nextInt(72, deckLength);
				Card temp = deck[j];
				deck[j] = deck[72];
				deck[35] = temp;
			}
